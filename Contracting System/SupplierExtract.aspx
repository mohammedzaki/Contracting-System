<%@ Page Title="" Language="C#" MasterPageFile="~/Users.Master" AutoEventWireup="true" CodeBehind="SupplierExtract.aspx.cs" Inherits="Contracting_System.SupplierExtract" %>
        ...
        $(function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#grid_ExtractDetails").html('<tr class="ItemRow"><td class="style16"><select name="WorkItem" class="b_width addrow" onchange="onChangeItem(this)"><option></option></select></td><td class="style22"><input type="text" name="Price" class="s_width addrow" onchange="onChangePrice(this)"/></td><td class="style18"><input type="text" name="LastDoneQTY" class="s_width addrow" readonly/></td><td class="style18"><input type="text" name="CurrentDoneQTY" class="s_width addrow" onchange="onChangePrice(this)"/></td><td class="style18"><input type="text" name="TotalDoneQTY" class="s_width addrow" readonly/></td><td class="style18"><input type="text" name="TotalPrice" class="s_width addrow" readonly/></td><td><input type="button" value="      " disabled/></td></tr>');
                    ...
                    $("#cbo_Suppliers").on("change", function () {
                        ...
                        $(subData).filter('main').find('View_SupplierExtractNewOrder').each(function () {
                        ...
                        $(subData).filter('main').find('View_EmpolyeePaid').each(function () {
                            // Last_Paid, PersonID, PersonTypeID
                            if ($(this).find('PersonID').text() == $("#cbo_Suppliers").val() && $(this).find('PersonTypeID').text() == "-3") {
        ...
        function SaveSupplierExtract() {
            if ($("#cbo_Employee").val() != '') {
                ...
                for (var i = 0; i < count; i++) {
                    ...
                    xmlSupplierExtract += "<LastExitExecutedQTY>" + parseInt(LastExecutedQTY) + "</LastExitExecutedQTY>";
                ...
                $.ajax({
                    ...
                    url: "SaveData.aspx/SaveSupplierExtract",
                    dataType: "json",
                    success: function (data) {
                        if (data.d > -1) {
