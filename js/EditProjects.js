...
function LoadPage() {
    $.ajax({
        ...
        success: function (data) {
            ...
            //<td class="hiddenElement">' + $(this).find('PK_ID').text() + '</td><td><input type="button" value="حذف" onclick="DeleteSubContractor(this)"/></td>
            $(subData).filter('main').find('Tbl_ProjectSubContractor').each(function () {
    ...
    if ($("#cbo_DailyWorker").val() != '') {
        $.ajax({
            ...
            success: function (data) {
                if ($(data).filter('main').find('Exception').text() == '') {
                    alert("تم الحفظ");
                    $("#grid_DailyWork").append('<tr class="ItemRow"><td class="style16"><input name="WorkerName" style="width: 180px" value="' + $("#cbo_DailyWorker option:selected").val() + '"/></td><td class="style22"><input type="text" name="WorkStartDate" value="' + $("#DailyWorkStartDate").val() + '"/></td><td class="style18"><input type="text" name="WorkWageDay" value="' + $("#txt_DailyWorkerWageDay").val() + '"/></td></tr>');
                } else {
                    alert($(data).filter('main').find('Exception').text());
                }
            },
    ...
    if ($("#cbo_Employees").val() != '') {
        $.ajax({
            ...
            success: function (data) {
                if ($(data).filter('main').find('Exception').text() == '') {
                    alert("تم الحفظ");
                    $("#grid_Employee").append('<tr class="ItemRow"><td class="style16"><input type="text" name="EmployeeName" style="width: 180px" value="' + $("#cbo_Employees option:selected").val() + '"/></td><td class="style22"><input type="text" name="EmployeeCode" readonly="readonly" value="' + $("#txt_EmployeeCode").val() + '"/></td><%--<td class="tbl_td_buttonRemove"><input type="button" value=" " style="background-color: silver;border: silver;"/></td>--%></tr>');
                } else {
                    alert($(data).filter('main').find('Exception').text());
                }
            },
